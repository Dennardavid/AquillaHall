...
import { nurserySubjects, nurseryAffectiveDoman } from "@/Types/Subjects";
...
export default function NurseryResultsUploadForm() {
  ...
  const calculateOverallTotal = () => {
    return nurserySubjects.reduce((total, subject) => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const subjectResults = nurserySubjects.reduce((acc, subject) => {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-white shadow-lg p-6 rounded-lg">
        ...
        <div>
          ...
          {nurseryAffectiveDoman.map((doman) => (
